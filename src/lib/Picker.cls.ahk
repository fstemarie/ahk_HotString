class Picker {
    _HWND, _TABS, _LVPICKER, _LVPICKER_HWND, _LBCATEGORIES
    , _BTNDOC, _BTNEDIT, _BTNRELOAD, _BTNQUIT, _BTNNEW
    , _BTNDELETE, _BTNRENAME, _BTNSAVE, _TVNOTES
    , _IMGLIST, _EDTNOTE, _notesCol, _category

    __New() {

    }

    Set_HotStrings(hsCol) {
        OutputDebug, % "-- Set_HotStrings() `n"
        if !hsCol or hsCol.Count() = 0
            return
        loop % hsCol.Count()
        {
    ...
    Gui_OnWMACTIVATEAPP(wParam, lParam, msg, hwnd) {
        if (hwnd = PICKER_HWND)
        {
    ...
    lbCategories_Update(categories) {
    ...
    tvNotes_OnMenu(itemName, itemPos, MenuName) {
    ...
    tvNotes_Recurse(path, currentID) {
        global notesCol
